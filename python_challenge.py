...
alphabet.append('a')
alphabet.append('b')
puzzle_string = """g fmnc wms bgblr rpylqjyrc gr zw fylb. rfyrq ufyr amknsrcpq ypc dmp. bmgle gr gl zw fylb gq glcddgagclr ylb rfyr'q ufw rfgq rcvr gq qm jmle. sqgle qrpgle.kyicrpylq() gq pcamkkclbcb. lmu ynnjw ml rfc spj.

"""

def puzzle_string_converter(string):
    new_string = ""
    for char in string:
        if char in alphabet:
...
print(puzzle_string_converter(puzzle_string))

## 3:



rare_string = """%%$@_$^__#)^)&!_+]!*@&^}@[@%]()%+$&[(_@%+%$*^@$^!+]!&_#)_*}{}}!}_]$[%}@[{_@#_^{*
@##&{#&{&)*%(]{{([*}@[@&]+!!*{)!}{%+{))])[!^})+)$]#{*+^((@^@}$[**$&^{$!@#$%)!@(&
+^!{%_$&@^!}$_${)$_#)!({@!)(^}!*^&!$%_&&}&_#&@{)]{+)%*{&*%*&@%$+]!*__(#!*){%&@++
!_)^$&&%#+)}!@!)&^}**#!_$([$!$}#*^}$+&#[{*{}{((#$]{[$[$$()_#}!@}^@_&%^*!){*^^_$^
]@}#%[%!^[^_})+@&}{@*!(@$%$^)}[_!}(*}#}#___}!](@_{{(*#%!%%+*)^+#%}$+_]#}%!**#!^_
...
#print(rare_string)
def rare_string_converter(string):
    ...
    while x < len(string):
        ...
        if string[x] not in string1:
            string1 += string[x]
        x += 1

    return string1

print(rare_string_converter(rare_string))
challenge_url = """http://www.pythonchallenge.com/pc/def/equality.html"""


big_bodyguard_string = """<!--
...
#print(big_bodyguard_string)
small_letter = ""
for x in range(len(big_bodyguard_string)):



    string = big_bodyguard_string[x: x + 9]
  ...
  #  print(string[x])
   ...
   # print(string)
    ...
    if (string[0].islower()) and (string[1].isupper()) and (string[2].isupper()) and string[3].isupper() and string[4].islower() and string[5].isupper() and string[6].isupper() and string[7].isupper() and string[8].islower():

      #  print(string)
        small_letter += string[4]
      #  print(string)
    # if (string[x].isupper()) and (string[x+1].isupper()):
    #     print(string)
    ...
    # if big_bodyguard_string[x].isupper():
    #     print(big_bodyguard_string[x])
...
challenge_url = """http://www.pythonchallenge.com/pc/def/linkedlist.php"""
# 4
...
while z < 401:
    ...
    url = ("""http://www.pythonchallenge.com/pc/def/linkedlist.php?nothing=%s""" %(new_number))
    ...
    # Print the content (in this example, it's HTML)
    print(html_content.decode('utf-8'))
    if(html_content.decode('utf-8')[0] != 'a'):
        information_array.append(html_content.decode('utf-8'))
    ...
    for x in range(len(html_content)):
        #print(html_content)
        print(url)
        try:


            a = int(html_content.decode('utf-8')[x])
            b = str(a)
        #    print(b)
            array_html_content.append(a)
            new_number += b
        except ValueError:
            continue
        if x == len(html_content) - 1:

            array_html_numbers.append(array_html_content)
            z+=1
            print(z)
            break

    print(array_html_numbers)
nothing = 12345
nothing = 44827
nothing = 45439
nothing = 94485
